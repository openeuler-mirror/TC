...
#### 内核kconfig调整
- 过去设置为`not set`的选项，凡属于驱动类、模块化功能类的，能设置为`=m`的都设置。因多个选项互斥n选一而导致的`not set`则保持不变。
...
考虑到未来可能需要在标准内核包之外支持多种内核变体（variant），如：

- `rt`：实时内核，需要增加PREEMPT_RT补丁。
- `rpi或类似开发板`：可能需要一些未上游化的vendor补丁。
- `desktop`：针对桌面体验进行优化。
- `debug`：尽可能多的开启debug配置，便于debug问题。

对基础方案中的内核包命名可做以下改进：

| 基础方案       | 进阶方案                |
| -------------- | ----------------------- |
| kernel-minimal | kernel-$variant-minimal |
| kernel-base    | kernel-$variant-base    |
| kernel-extras  | kernel-$variant-extras  |

例如：当前标准内核设定 `$variant = default`，则标准内核的子包分别为：kernel-default-minimal，kernel-default-base，kernel-default-extras。

当前内核包spec文件需在基础方案之上进一步修改，增加variant定义。 kernel-{devel,headers,source}等子包也需variant化。 为保证多个variant的spec文件同步，消灭重复多次手动修改多个variant的spec文件可能带来的问题，可考虑由单一模板文件自动生成多个variant spec文件的方式进行管理。本oEEEP中不再赘述，在未来提案中阐述。
